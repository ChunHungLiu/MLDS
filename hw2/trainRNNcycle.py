...
MODEL = "_".join([ "DATA",DATA, \
                   ...
                   #"DROPOUT",str(DROPOUT),
                   "L_RATE_ANNEALED",str(LEARNING_RATE_ANNEALED) ])
...
for epoch in range(MAX_EPOCH):
    ...
    for batch_num in range(0,int(train_data_length/BATCH_SIZE)):
        ...
        for id_utter in pickList[ p_s:p_e ]:
            ...
            extended_in = None
            if utter_len <= MAX_FRAME:
                end = start + utter_len
                extended_in = [ (i-0.4)*5 for i in mem_pgram[start:end,:]]
                state_vec = [ get_ph48_vec( PhoneIdx[PhoneState[i]] ) \
                    for i in mem_label[start:end] ]
                while len(extended_in) < MAX_FRAME:
                    append_len = utter_len
                    if MAX_FRAME - len(extended_in) < utter_len:
                        append_len = MAX_FRAME - len(extended_in)

                    end = start + append_len
                    extended_in += [ (i-0.4)*5 \
                        for i in mem_pgram[start:end,:]]
                    state_vec += [get_ph48_vec( PhoneIdx[PhoneState[i]] ) \
                        for i in mem_label[start:end] ]
                extended_out = [ state_vec ]
            else:
                pointer = np.random.randint(utter_len-MAX_FRAME)
                start += pointer
                end   =  start + MAX_FRAME
                extended_in  = [ (i-0.4)*5 for i in mem_pgram[start:end,:]]
            ...
            utter_mask = [ np.ones((MAX_FRAME,1)) ]
        ...
        assert x_seq.shape == y_hat_seq.shape
    ...
    if (epoch+1 != MAX_EPOCH) and err_range < 0.2:# and (epoch+1 == 50):
    ...
    if VAL_SET_RATIO != 1:
        ...
        #if flag == 1:
        #    prev_3 = prev_2 = prev_cost = float('inf')

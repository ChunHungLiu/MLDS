...
class HMM(object):
    def __init__(self,state_num,observe_num,min_duration = 1, max_duration = 5):
        ...
        self.trans_prob = np.zeros(( self.state_num, self.state_num )) + 1e-100
    ...
    def read(self,pgram,label,IDs,IDs_utter):
        ...
        self.trans_prob = subtract_by_col( np.log(self.trans_prob), np.log(phone_count) )
        #self.trans_prob = np.log(divide_by_col(self.trans_prob, phone_count))
    ...
    def test(self, pgram, IDs, IDs_utter):
        ...
        def split_by_utter(pgram,IDs_utter):
            ...
            for utter in IDs_utter:
                cur_idx += utter[1]
                utter_pgram.append(np.log(pgram[prev_idx:cur_idx]))
                prev_idx = cur_idx
            return utter_pgram
        ...
        for utter in split_by_utter(pgram, IDs_utter):
            tStart = time.time()
            self.result.append( viterbi(np.log(utter)) )
...
def save(IDs_utter, result,min_duration, max_duration):
    def map2seq(sequence):
        ...
        def fix_seq(seq):
            smooth_y = []
            smooth_y1 = []
            smooth_y1.append(seq[0])
            for i in xrange(1,len(seq)-1):
                if seq[i-1] == seq[i+1] and \
                    seq[i]   != seq[i-1]:
                    smooth_y1.append( seq[i-1] )
                elif seq[i] != seq[i-1] and \
                    seq[i] != seq[i+1]:
                    # discard
                    continue
                    #final_seq.append( seq[i-1] )
                else:
                    smooth_y1.append( seq[i] )
            ...
            if not all( s == 'sil' for s in smooth_y ):
                while smooth_y[0] == 'sil':
                    smooth_y.pop(0)
                while smooth_y[-1] == 'sil':
                    smooth_y.pop(-1) 
            return smooth_y
    ...
    MODEL = "_".join(['Hmm','hsmm','log',str(min_duration),str(max_duration)])
    
    PREDICTION_ROOT ='./sl_result/phone_sequence/'

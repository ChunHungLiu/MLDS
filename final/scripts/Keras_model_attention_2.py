...
def keras_model( image_vector=1000 , word_vector=300 ):
    ...
    DNN_units   = [2048 , 2048 , 2048, 1024 , 1024, 1024 ,512, 512]

    model = Sequential()
    
    attention_base = Matched_filter_2(input_dim = 2800)
    ...
    layer_DNN_7     = Dense(DNN_units[6] , init = 'uniform')
    layer_DNN_7_act = Activation('relu')
    layer_DNN_7_dro = Dropout(p=0.5)
    
    layer_DNN_8     = Dense(DNN_units[7] , init = 'uniform')
    layer_DNN_8_act = Activation('relu')
    layer_DNN_8_dro = Dropout(p=0.5)
    ...
    model.add(layer_DNN_7)
    model.add(layer_DNN_7_act)
    model.add(layer_DNN_7_dro)
    model.add(layer_DNN_8)
    model.add(layer_DNN_8_act)
    model.add(layer_DNN_8_dro)
    model.add(layer_out)

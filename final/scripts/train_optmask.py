...
MODEL_FILE_NAME = '../models/optmask_8_8_1.19'
EXP_FILE_NAME = '../exp/optmask_8_8_1.19'
#===== load training data =====
data_prefix = '../Data/pkl/'
stop = '_with_stop_word'
paths = [
    'img_q_id_train',
    'question_train',
    'choices_train',
...
QUESTION_PKL = pickle.load(open(data_prefix+paths[1]+stop+'.pkl','rb'))
CHOICE_PKL = pickle.load(open(data_prefix+paths[2]+stop+'.pkl','rb'))
ANSWER_PKL = pickle.load(open(data_prefix+paths[3]+stop+'.pkl','rb'))
ANS_SOL_PKL = pickle.load(open(data_prefix+paths[4]+stop+'.pkl','rb'))
ANNO_PKL = pickle.load(open(data_prefix+paths[5]+stop+'.pkl','rb'))
...
model = Keras_optmask.keras_model()
...
for epoch in xrange(MAX_EPOCH):
    ...
    for batch_num in xrange(0,int(len(pickList)/BATCH_SIZE)):
        ...
        for idx in pickList[start:end]:
            for idx_sh in xrange(SHUFFLE_NUM):
        ...
        cost += model.train_on_batch( [ data,data ], solution )[0]
    ...
    if (epoch+1)%EXP_EPOCH==0:
        ...
        for batch_num in xrange(0,int(len(pickList)/BATCH_SIZE)):
            ...
            solution        = np.vstack(batched_sol)
            data = np.hstack([image_input,question_input,ans1_input,ans2_input,ans3_input,ans4_input,ans5_input])
            prediction = model.predict_on_batch( [data,data])

...
def keras_model():
    ...
    model_filter.add( Dense( 2048 , input_dim = 2800 ) )
    model_filter.add( Activation('relu') )
    model_filter.add( Dropout(p=0.5) )
    '''
    model_filter.add( Dense( 1028) )
    model_filter.add( Activation('relu') )
    model_filter.add( Dropout(p=0.5) )
    '''
    model_filter.add( Dense( 1000 ) )
    # TO exp
...
for n in range(100):
    ...
    print model.train_on_batch( [ input_data , input_data] , solution )
    #print model_1.train_on_batch(ans1_input , solution)

print model.predict_on_batch( [ input_data , input_data ])
